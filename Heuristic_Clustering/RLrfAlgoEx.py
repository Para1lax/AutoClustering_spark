...
class RLrfAlgoEx:
    ...
    def __init__(self, spark_df, metric='sil', params=None, expansion=5000):
        self.spark_df, self.metric = spark_df, metric
        self.run_num = np.zeros(params.num_algos, dtype=np.int)
        ...
        for i in range(self.params.num_algos):
            self.th.append(RLthreadRFRS(spark_df, algorithm_name=self.clu_algos[i], params=self.params,
    ...
    def apply(self, arm, file, iteration_number, remaining_time=None, current_time=0):
        ...
        if reward < self.best_val:
            ...
            self.best_labels = th.predictions
            self.same_res_counter = 0
        else:
            self.same_res_counter += 1

        log_iteration = [iteration_number, self.metric, self.best_val, self.best_algo,
                         self.clu_algos[arm], reward, current_time + run_spent]
