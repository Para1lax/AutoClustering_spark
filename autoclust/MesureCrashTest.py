...
class MeasureCrashTest(unittest.TestCase):
    """
    ...
    def setUpClass(cls):
        ...
        df = pyspark.SQLContext(sc).read.csv(MeasureCrashTest.csv_path, header=True, inferSchema=True)

        df = HD.make_id(HD.assemble(df, MeasureCrashTest.columns[0])).withColumnRenamed(MeasureCrashTest.columns[1], 'labels')
        cls.true_df, cls.random_df = df, df.withColumn('labels', round(rand() * (MeasureCrashTest.k - 1)).cast('int'))

    def __invoke_measure(self, algorithm):
        measure = Measure.make(algorithm, distance=MeasureCrashTest.distance)
