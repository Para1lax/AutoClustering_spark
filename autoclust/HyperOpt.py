...
class HyperOpt:
    ...
    def dbscan_config(self):
        return dict(
            eps=(HyperOpt.FLOAT, (0.1, 0.7)),
            min_pts=(HyperOpt.INT, (2, 6)),
            distance=(HyperOpt.CATEGORY, (['euclidean', 'manhattan'],)),
        )

    def kmeans_config(self):
        return dict(
            k=(HyperOpt.INT, (2, self.ds.max_clusters)),
            initMode=(HyperOpt.CATEGORY, (['random', 'k-means||'],)),
            initSteps=(HyperOpt.INT, (1, 5)),
            maxIter=(HyperOpt.INT, (5, 50)),
            distanceMeasure=(HyperOpt.CATEGORY, (['euclidean', 'cosine'],))
        )
    ...
    def gaussian_mixture_config(self):
        return dict(
            k=(HyperOpt.INT, (2, self.ds.max_clusters)),
            maxIter=(HyperOpt.INT, (5, 50)),
            tol=(HyperOpt.FLOAT, (1e-6, 0.1)),
            aggregationDepth=(HyperOpt.INT, (2, 15))
        )
    ...
    def bisecting_kmeans_config(self):
        return dict(
            ...
            maxIter=(HyperOpt.INT, (5, 50)),
            distanceMeasure=(HyperOpt.CATEGORY, (['euclidean', 'cosine'],)),
            minDivisibleClusterSize=(HyperOpt.FLOAT, (0.01, 1.0))
        )
...
class SmacOpt(HyperOpt):
    """
    Wrapper for SMAC3 optimiser(https://automl.github.io/SMAC3/master/). Using SMAC4HPO facade
    """
    def __init__(self, algorithm, ds):
        HyperOpt.__init__(self, algorithm, ds)
        from smac.runhistory.runhistory import RunHistory
        from ConfigSpace import ConfigurationSpace
        import ConfigSpace
        self.hyper_map = {  # map from our common params to ConfigSpace hyper parameters
            HyperOpt.INT: ConfigSpace.UniformIntegerHyperparameter,
            HyperOpt.FLOAT: ConfigSpace.UniformFloatHyperparameter,
            HyperOpt.CATEGORY: ConfigSpace.CategoricalHyperparameter,
        }
...
class OptunaOpt(HyperOpt):
    ...
    def __init__(self, algorithm, ds):
        ...
        self.__hyper_map__ = {  # map from our common params to Optuna hyper parameters
            HyperOpt.INT: optuna.Trial.suggest_int,
            HyperOpt.FLOAT: optuna.Trial.suggest_float,
            HyperOpt.CATEGORY: optuna.Trial.suggest_categorical
        }
...
get_optimiser = {'smac': SmacOpt, 'optuna': OptunaOpt}
